...
- En macOS usa exclusivamente los scripts `.sh` y herramientas nativas (`/bin/sh`, `curl`, `pbpaste`, `osascript`). No instales PowerShell, Homebrew, `jq` ni dependencias. Copia `scripts/key.sh` a `$HOME/.config/learnforce/lfkey`, aplica `chmod +x` y registra el alias `lfkey` en el perfil de la shell activa. Preserva siempre el perfil y no dupliques la definición.
